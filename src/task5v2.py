...
class Trader:
    ...
    def analyze_counterparty(self, product: str, own_trades: List[Trade], state_data: TraderState, current_timestamp: int) -> Dict[str, float]:
        ...
        for trade in own_trades[-3:]:  # Process only the last 3 trades
        ...
        return state_data.counterparty_avg_price[product]
        ...
        for product in state.order_depths:
            ...
            if counter_party_prices:
                ...
                if avg_counter_price > fair_value + spread_limit:
                    sell_price = min(sell_price + 1, floor(avg_counter_price * 1.01))  # Slightly above counterparty avg
                elif avg_counter_price < fair_value - spread_limit:
                    buy_price = max(buy_price - 1, ceil(avg_counter_price * 0.99))  # Slightly below counterparty avg

            # Calculate quantities
            max_buy_qty = position_limit - current_position
            max_sell_qty = -(position_limit + current_position)
            order_qty = min(5, max(1, position_limit // 4))
